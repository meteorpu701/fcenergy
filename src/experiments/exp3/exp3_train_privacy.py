# src/experiments/exp3/exp3_train_privacy.py
...
def _parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--tokenize_hubs", action="store_true",
                help="Replace hub identifiers with deterministic tokens in logs and client keys.")
    ap.add_argument("--token_salt", default=None,
                help="Salt for hub tokenisation. If omitted, uses env FC_TOKEN_SALT or a fixed default.")
    ap.add_argument("--mask_clip", type=float, default=0.0,
                help="If >0, clip features to [-mask_clip, mask_clip] after scaling.")
    ap.add_argument("--mask_bits", type=int, default=0,
                help="If >0, quantise features to mask_bits using symmetric uniform quantisation after scaling.")
    ap.add_argument("--privacy_sigma", type=float, default=0.0,
                help="Stddev of Gaussian noise added to client delta (σ=0 disables).")
    ap.add_argument("--canary_frac", type=float, default=0.0, help="Fraction of a client's samples replaced with canaries (0 disables).")
    ap.add_argument("--canary_client", default=None, help="Which *raw hub name* gets the canary (e.g. FIN). If None, use first training hub.")
    ap.add_argument("--canary_y", type=float, default=3.0, help="Canary label value in y-space (log return).")
    ap.add_argument("--canary_seed", type=int, default=0, help="Seed for selecting which rows become canaries.")
    ap.add_argument("--canary_mode", default="replace", choices=["replace","append"], help="Replace existing rows or append new rows.")
    ap.add_argument("--update_clip", type=float, default=0.0,
                help="Global L2 clip for client delta before adding privacy noise.")
    ap.add_argument("--canary_eval_every", type=int, default=1,
                help="Evaluate canary every N rounds (1 = every round).")
...
# DATA HELPERS
# ============================================================
def _inject_canary_replace(
    train_df: pd.DataFrame,
    train_hubs: list[str],
    X_train_all: np.ndarray,
    y_train_all: np.ndarray,
    canary_client: str | None,
    canary_frac: float,
    canary_y: float,
    seed: int,
):
    if canary_frac <= 0:
        return X_train_all, y_train_all, {"canary_n": 0, "canary_client": None}

    rng = np.random.default_rng(int(seed))

    target_hub = canary_client or train_hubs[0]
    if target_hub not in train_hubs:
        target_hub = train_hubs[0]

    idx = train_df.index[train_df["hub"] == target_hub].to_numpy()
    if len(idx) == 0:
        return X_train_all, y_train_all, {"canary_n": 0, "canary_client": target_hub}

    n = max(1, int(np.floor(float(canary_frac) * len(idx))))
    chosen = rng.choice(idx, size=n, replace=False)

    d = X_train_all.shape[1]
    x_canary = np.zeros((n, d), dtype=float)     # signature in *scaled space*
    y_canary = np.full((n,), float(canary_y), dtype=float)

    X_new = X_train_all.copy()
    y_new = y_train_all.copy()
    X_new[chosen] = x_canary
    y_new[chosen] = y_canary

    return X_new, y_new, {"canary_n": int(n), "canary_client": target_hub}

def _hub_token(hub: str, salt: str) -> str:
    """
    Deterministic token for a hub string.
    Salt should be constant for a given experiment run (set via CLI or env).
    """
    msg = (salt + "::" + str(hub)).encode("utf-8")
    # short-but-unique token for logs/keys
    return hashlib.sha256(msg).hexdigest()[:10]

def _clip_and_quantise(X: np.ndarray, clip: float, bits: int) -> np.ndarray:
    ...
    if bits and bits > 0:
        # symmetric uniform quantisation in [-clip, clip] if clip>0
        # if clip==0, derive a scale from data (still deterministic for given X)
        if clip and clip > 0:
            s = float(clip)
        else:
            s = float(np.max(np.abs(X))) if np.isfinite(X).any() else 1.0
            if s <= 0:
                s = 1.0

        qmax = (2 ** (bits - 1)) - 1  # e.g. 7 for 4-bit signed
        Xq = np.round((X / s) * qmax)
        Xq = np.clip(Xq, -qmax, qmax)
        X = (Xq / qmax) * s
...
def _fit_transform_train_only(train_df: pd.DataFrame,
                              ...
                              feat_cols: List[str],mask_clip: float = 0.0, mask_bits: int = 0) -> tuple[np.ndarray, np.ndarray]:
    ...
    # ---- masking (privacy level 2) ----
    if (mask_clip and mask_clip > 0) or (mask_bits and mask_bits > 0):
        X_train = _clip_and_quantise(X_train, clip=float(mask_clip), bits=int(mask_bits))
        X_test  = _clip_and_quantise(X_test,  clip=float(mask_clip), bits=int(mask_bits))
...
def run_one_test_hub(df: pd.DataFrame,
    ...
    # ----------------------------
    # Hub tokenisation (Option 1)
    # ----------------------------
    tokenize = bool(getattr(args, "tokenize_hubs", False))
    salt = getattr(args, "token_salt", None) or os.environ.get("FC_TOKEN_SALT") or "fc_default_salt"

    hub_to_token = {h: _hub_token(h, salt) for h in hubs} if tokenize else {h: h for h in hubs}
    token_test_hub = hub_to_token[test_hub]
    token_train_hubs = [hub_to_token[h] for h in train_hubs]
    token_to_hub = {hub_to_token[h]: h for h in hubs}
    ...
    X_train_all, X_test = _fit_transform_train_only(
    train_df, test_df, feat_cols,
    mask_clip=float(getattr(args, "mask_clip", 0.0)),
    mask_bits=int(getattr(args, "mask_bits", 0)),)
    ...
    # ----------------------------
    # Canary injection (replace-mode)
    # ----------------------------
    canary_frac = float(getattr(args, "canary_frac", 0.0))
    canary_client = getattr(args, "canary_client", None)
    canary_y = float(getattr(args, "canary_y", 3.0))
    canary_seed = int(getattr(args, "canary_seed", 0))
    canary_mode = str(getattr(args, "canary_mode", "replace")).lower()

    canary_meta = {"canary_n": 0, "canary_client": None}
    if canary_mode == "replace" and canary_frac > 0:
        X_train_all, y_train_all, canary_meta = _inject_canary_replace(
            train_df=train_df,
            train_hubs=train_hubs,
    ...
    for h in train_hubs:
        t = hub_to_token[h]   # token key
        idx = train_df.index[train_df["hub"] == h].to_numpy()
        X_h = X_train_all[idx]
        y_h = y_train_all[idx]
        clients[t] = _make_loader(X_h, y_h, batch_size=int(args.batch_size), shuffle=True)
    ...
    clients_per_round = min(clients_per_round, len(train_hubs))  # raw hubs count

    # If tokenisation is enabled, the number of client keys equals token_train_hubs
    # (same cardinality, but keep this defensive in case you later filter clients)
    if tokenize:
        clients_per_round = min(clients_per_round, len(token_train_hubs))
    else:
    ...
    if algo == "scaffold":
        ...
        c_locals = {t: {k: v.clone() for k, v in c0.items()} for t in token_train_hubs}
    best = float("inf")
    best_state = None
    no_improve = 0
    logs: list[dict] = []
    ...
    for r in range(1, int(args.rounds) + 1):
        chosen = rng.choice(token_train_hubs, size=clients_per_round, replace=False)
        ...
        for h in chosen:
            ...
            if algo == "scaffold":
                ...
                update = client_fit_fedavg(
                    ...
                    epochs=int(args.local_epochs),
                    device="cpu",
                    sigma=float(getattr(args, "privacy_sigma", 0.0)),
                    clip=float(getattr(args, "update_clip", 0.0)),
                )
        ...
        fixed_canary_rounds = {1, 5, 10, 20, 50, 100, 200}  # pick what you want
        canary_every = int(getattr(args, "canary_eval_every", 1))
        do_canary_eval = (canary_every > 0) and (r % canary_every == 0)
        #do_canary_eval = (r in fixed_canary_rounds)

        # Canary evaluation (same signature as injected)
        rmse_canary = np.nan
        if getattr(args, "canary_frac", 0.0) > 0 and do_canary_eval:
            x_c = np.zeros((canary_meta["canary_n"], X_train_all.shape[1]), dtype=np.float32)
            y_c = np.full((canary_meta["canary_n"],), float(canary_y), dtype=np.float32)
            with torch.no_grad():
                pred_c = model(torch.tensor(x_c)).detach().cpu().numpy().reshape(-1)
            rmse_canary = float(np.sqrt(np.mean((pred_c - y_c) ** 2)))

        logs.append({
            # identifiers / knobs
            "test_hub": token_test_hub if tokenize else test_hub,
            "test_hub_raw": test_hub if tokenize else None,   # optional: keep for debugging, can remove later
            "tokenize_hubs": int(tokenize),
            "token_salt_set": int(getattr(args, "token_salt", None) is not None),
            "algo": algo,
            ...
            "mask_clip": float(getattr(args, "mask_clip", 0.0)),
            "mask_bits": int(getattr(args, "mask_bits", 0)),
            "privacy_sigma": float(getattr(args, "privacy_sigma", 0.0)),
            "canary_n": int(canary_meta.get("canary_n", 0)),
            "canary_client": canary_meta.get("canary_client", None),
            "canary_rmse": float(rmse_canary) if not np.isnan(rmse_canary) else np.nan,
            "canary_frac": float(canary_frac),
            "canary_round_eval": int(do_canary_eval),
        })
    ...
    if not out.empty:
        ...
        print(f"[BEST] {token_test_hub if tokenize else test_hub} algo={algo} local_epochs={int(best_row['local_epochs'])} "
